...
| **Model Selector UX & Anti-Collision** | `custom.css` | CSS override | Implements **Laws of UX** optimization: widens dropdown menu (`32rem` / 512px; Hick's Law), enforces dynamic button height with 44px min-target (`min-h-[2.75rem]`; Fitts's Law & Law of Proximity), provides clean multi-line wrapping with 1.35 line-height (Jakob's Law), and unclamps active model pill (`max-w-56` -> `50rem`) so all personas and clearance tiers (`Tier A`, `Tier B`, `Tier C`) render with zero text collision. |
