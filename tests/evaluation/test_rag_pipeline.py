...
def test_subcampus_skab_s_access_and_isolation(rag_engine):
    """Ensures a teacher at SKAB S accesses Sentinel cluster documents but is isolated from Trident."""
    skab_s_teacher = {
        "name": "Mubanga Mwale",
        "campus": "SKAB S",
        "clearance": "staff",
    ...
    # Should retrieve Sentinel cluster student submission
    sentinel_docs = rag_engine.retrieve(
        query="Summarize student submission #8812 regarding network security",
        user_session=skab_s_teacher,
        k=3
    )
    assert len(sentinel_docs) > 0
    assert any(d.metadata.get("id") == "DOC-SUBMISSION-005" for d in sentinel_docs)

    # Should NOT retrieve Trident financial records
    trident_docs = rag_engine.retrieve(
        query="Trident campus Q3 financial ledger and expenditure allocation",
        user_session=skab_s_teacher,
        k=5
    )
    for doc in trident_docs:
        assert doc.metadata.get("campus") != "trident"

def test_subcampus_tcl_access_and_isolation(rag_engine):
    """Ensures a staff member at TCL accesses Trident docs but is blocked from Sentinel staff docs."""
    tcl_staff = {
        "name": "Mwamba Banda",
        "campus": "TCL",
        "clearance": "staff",
    ...
    # Should NOT retrieve Sentinel-specific staff code of conduct
    sentinel_docs = rag_engine.retrieve(
        query="Sentinel Kabitaka staff code of conduct and briefing",
        user_session=tcl_staff,
        k=5
    )
    for doc in sentinel_docs:
        assert doc.metadata.get("campus") != "sentinel"

def test_frontier_nkisu_isolation(rag_engine):
    """Ensures Frontier Nkisu staff cannot access Sentinel or Trident campus files."""
    frontier_staff = {
    ...
    docs = rag_engine.retrieve(
        query="Staff code of conduct or financial ledger",
        user_session=frontier_staff,
        k=5
    )
    for doc in docs:
        doc_campus = doc.metadata.get("campus", "").lower()
        assert doc_campus not in ["sentinel", "trident"], f"Frontier staff accessed {doc_campus} document"
